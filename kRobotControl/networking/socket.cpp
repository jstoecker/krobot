...
namespace net{
        ...
        Socket::setReuseAddress( bool on )
        {
            int ison = on;
            return setsockopt( m_socket, SOL_SOCKET,
                               SO_REUSEADDR, (void*)&ison, sizeof( int ));
        }

        int
        Socket::setMulticastTTL( unsigned char ttl )
        {            
            return setsockopt( m_socket, IPPROTO_IP,
                               IP_MULTICAST_TTL, (void*) &ttl, sizeof(ttl));
        }

        int
        Socket::joinMulticastGroup(const char* multicast_ip)
        {
#ifdef SO_MULTICAST
            ip_mreq mc_req;
            mc_req.imr_multiaddr.s_addr = inet_addr(multicast_ip);
            mc_req.imr_interface.s_addr = htonl(INADDR_ANY);
            return setsockopt( m_socket, IPPROTO_IP, IP_ADD_MEMBERSHIP, (void*) &mc_req, sizeof(mc_req));
#else
            errno = EPERM;
            return -1;
#endif
        ...
        int
        Socket::leaveMulticastGroup(const char* multicast_ip)
        {
#ifdef SO_MULTICAST
            ip_mreq mc_req;
            mc_req.imr_multiaddr.s_addr = inet_addr(multicast_ip);
            mc_req.imr_interface.s_addr = htonl(INADDR_ANY);
            return setsockopt(m_socket, IPPROTO_IP, IP_DROP_MEMBERSHIP, (void*) &mc_req, sizeof(mc_req));
#else
            errno = EPERM;
            return -1;
#endif          
